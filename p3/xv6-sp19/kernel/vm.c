...
#define SHMEM_PAGES_COUNT 4

void* shmem_address[SHMEM_PAGES_COUNT];
...
copyuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = proc->stack_top; i < USERTOP; i += PGSIZE){
    if((pte = walkpgdir(pgdir, (void*)i, 0)) == 0)
      panic("copyuvm: pte should exist");
    if(!(*pte & PTE_P))
      panic("copyuvm: page not present");
...
shmem_init(void)
{
  for ( int i = 0; i < SHMEM_PAGES_COUNT; i++ ) {
    if ( !(shmem_address[i] = kalloc()) ) {
...
{
  if ( page_number < 0 || page_number >= SHMEM_PAGES_COUNT ) {
    return NULL;
  }
  ...
  void* to_map = (void*) USERTOP - ((proc->shmem_count + 1) * PGSIZE);
  
  if ( proc->sz >= (int) to_map ) {
    return NULL;
  }
  else {
    cprintf("if you see this maybe delete");
  }
  
  if ( mappages(proc->pgdir, to_map, PGSIZE, PADDR(shmem_address[page_number]), PTE_W|PTE_U) != 0 ) {
    return NULL;
  }
  
  proc->shmem_count++;
  proc->shmems[page_number] = to_map;
